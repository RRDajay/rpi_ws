...
#define RESOURCES_DIR "resource"

int http_server_main_thread();
...
static int s_signo = 0;                 // Signal number to exit on
...
int http_server_main_thread(int argc, char *argv[])
{
    char path[MG_PATH_MAX] = "";
    struct mg_mgr mgr;
    ...
    if(!strlen(path)) {
        char temp_path[MG_PATH_MAX];
        snprintf(temp_path, sizeof(temp_path), "%s", argv[0]);
        dirname(temp_path);
        realpath(temp_path, path);
        strncat(path, "/" RESOURCES_DIR "/", sizeof(path) - strlen(path));
        s_root_dir = path;
    }
